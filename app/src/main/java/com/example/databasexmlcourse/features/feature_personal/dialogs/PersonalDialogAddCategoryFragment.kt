...
import com.example.databasexmlcourse.features.login.LoginViewModel
import dagger.hilt.android.AndroidEntryPoint
import kotlinx.coroutines.flow.onEach

@AndroidEntryPoint
class PersonalDialogAddCategoryFragment : DialogFragment() {
    private var _binding: DialogFragmentAddCategoryMenuBinding? = null
    private val binding get() = _binding!!

    private val viewModel: PersonalDialogAddCategoryViewModel by viewModels()
    ...
    private fun handleState(state: PersonalDialogAddCategoryViewModel.State) {
        updateSaveButtonState(state.isSaveButtonEnable)
    }

    private fun handleActions(action: PersonalDialogAddCategoryViewModel.Actions) {
        when (action) {
            is PersonalDialogAddCategoryViewModel.Actions.GoBack -> dismiss()
            is PersonalDialogAddCategoryViewModel.Actions.ShowFailedText -> binding.addCategoryError.visibility = View.VISIBLE
        }
    }
