...
class MenuDialogFragment : DialogFragment() {
    private var _binding: DialogFragmentMenuBinding? = null
    private val binding get() = _binding!!

    private val viewModel: MenuDialogViewModel by viewModels()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        _binding = DialogFragmentMenuBinding.inflate(inflater, container, false)
    ...
    private fun handleActions(action: MenuDialogViewModel.Actions) {
        when (action) {
            is MenuDialogViewModel.Actions.OpenCategoryDialog -> MenuDialogRecyclerFragment().show(childFragmentManager, "menu category view fragment")
